...
class IndexController extends HomeBaseController {
	...
	public function index() {
	   ...
	   //投资月排行榜
	  $month_load_top_list = M('loan_load as ld')->join('__USERS__ AS u on u.id=ld.user_id ')->where("ld.create_time>".(time()-60*60*24*30))->group('user_id ')->limit(10)->field('sum(money),u.user_login')->order('money desc' )->select();
	  $this->assign('month_load_top_list',$month_load_top_list);
	  //投资周排行榜
	  $week_load_top_list = M('loan_load as ld')->join('__USERS__ AS u on u.id=ld.user_id ')->where("ld.create_time>".(time()-60*60*24*7))->group('user_id ')->limit(10)->field('sum(money),u.user_login')->order('money desc' )->select();
	  $this->assign('week_load_top_list',$week_load_top_list);
	  //投资日排行榜
	  $day_load_top_list = M('loan_load as ld')->join('__USERS__ AS u on u.id=ld.user_id ')->where("ld.create_time>".(time()-60*60*24*1))->group('user_id ')->limit(10)->field('sum(money),u.user_login')->order('money desc' )->select();
    ...
    public function lists(){
       ...
       //借款期限
        ...
        //借款期限
        $months_type=array(array('id'=>0,'name'=>'不限'),array('id'=>1,'name'=>'3个月一下'),array('id'=>2,'name'=>'3-6 个月'),array('id'=>3,'name'=>'6-9 个月'),array('id'=>4,'name'=>'9-12 个月'),array('id'=>5,'name'=>'12 个月以上'));
