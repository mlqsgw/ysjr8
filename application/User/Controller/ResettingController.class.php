...
class ResettingController extends HomeBaseController {
	function index(){
		
		$this->display("/Public/resetting");
	}
	function fs()
	{
		...
		$data["user_email"]=$em;
		$list=$users_model->where($data)->find();
		if($list>=1)
		{
			$str=$list["user_email"]."+".$list["user_pass"];
			...
			$mail->Username = "hnyusenshiye@163.com";//你的邮箱账号
			$mail->Password = "zjumtejblpzwpsms";//你的邮箱密码
			$mail->From = "hnyusenshiye@163.com";//你的邮箱账号
			//$mail->FromName = get_option('blogname');
			$to = $_POST["email"];//收件人
			$mail->AddAddress($to);
			$mail->Subject = "取回密码说明";//主题
			$mail->Body = "取回密码说明<br>您收到这封邮件，是由于这个邮箱地址在 煜商金融 被登记为用户邮箱，且该用户请求使用 Email 密码重置功能所致。<br>--------------------------------------<br><b>重要</b><br>--------------------------------------<br>如果您没有提交密码重置的请求或不是 煜商金融 的注册用户，请立即忽略并删除这封邮件。只有在您确认需要重置密码的情况下，才需要继续阅读下面的内容。<br>--------------------------------------<br><b>密码重置说明</b><br>--------------------------------------<br>在下面的链接所打开的页面中输入新的密码后提交，您即可使用新的密码登录网站了<br><a href='http://www.ysjr8.com/index.php/User/Resetting/retting/id/$str'>http://www.ysjr8.com/index.php/User/Resetting/retting/id/$str/</a><br>(如果上面不是链接形式，请将该地址手工粘贴到浏览器地址栏再访问)<br>煜商金融";//正文
			$mail->WordWrap = 80;
			//$mail->AddAttachment("f:/test.png"); //可以添加附件
			$mail->IsHTML(true);
			$mail->Send();
			$this->success("密码重置已发往您的邮箱，请查看");		
		}else{
			$this->error("您输入的邮箱有误");	
		}
	}
	function retting()
	{
		...
		$str=explode("+",$id);
		$users=M("Users");
		$data["user_pass"]=$str[1];
		$data["user_email"]=$str[0];
		$r=$users->where($data)->find();
		if($r>=1)
		{
			$this->assign("pass",$str[1]);
			$this->assign("em",$str[0]);
	...
	function update()
	{
		...
		$where["user_pass"]=$_POST["id"];
		$where["user_email"]=$_POST["em"];
		
		if($users->where($where)->save($data))
		{
