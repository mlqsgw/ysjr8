...
<div id="deal-intro" style="border:none;">
	<div class="pos f_yahei">
		<div class="pos_link f_yahei">
			...
			<a href="<?php echo UU('Loan/index/lists');?>">投资列表</a>
			<span style="font-weight:bold;"> &gt;</span> 
			<a>我要投标</a>
		</div>
	</div>
			...
			<div class="clearfix" style="width:930px">
				...
				<table width="100%" cellpadding="0" cellspacing="0" class="bid-table">
					...
					<tr>
						...
						<td class="f_red bg"><?php echo ($loan["borrow_amount_format"]); ?></td>
					</tr>
					<tr>
						<td class="tit tr w100">借款年利率：</td>
						<td class="f_red"><?php echo ($loan["rate"]); ?> %</td>
					</tr>
					<tr>
						<td class="tit tr w100">借款期限：</td>
						<td class="f_red bg"><?php echo ($loan["repay_time"]); ?>个月</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><?php echo ($loan["repay_type_name"]); ?></td>
					</tr>
					<tr>
						...
						<td class="bg">
							<div class="blueProgressBar progressBar f_l" style="margin-top:16px">
		                        <div class="p">
		                        	<div class="c" style="width:<?php echo ($loan["progress_point"]); ?>%;"></div>
		                       	</div>
		                    </div>
							<div class="f_l pl5 pt5"><?php echo (round($loan["progress_point"],"2")); ?>%</div>
						</td>
					</tr>
					<tr>
						<td class="tit tr btm w100">剩余投标金额：</td>
						<td class="f_red btm"><?php echo ($loan["need_money"]); ?></td>
					</tr>
				</table>
				<div class="blank"></div>
				<table width="100%" cellpadding="0" cellspacing="0" class="bid-table">
					<tr>
						...
						<td class="f_red">
							<?php echo ($available_funds); ?>
							&nbsp;&nbsp;
							<a href="<?php echo UU('User/Money/index/',array('act'=>'incharge'));?>" target="_blank">我要充值</a>
						</td>
					</tr>
					<tr>
						<td class="tit tr w100 ">投标金额：</td>
						<td class="f_red bg ">
							<input class="f-input f-input90"  autocomplete="off" name="bid_money" id="J_BIDMONEY" /> 元&nbsp;&nbsp;<span class="f_dgray">(最低投标金额为<?php echo ($loan["min_loan_money"]); ?>元<?php if($loan["max_loan_money"] > 0): ?>,最高投标金额为<?php echo ($loan["max_loan_money"]); ?>元<?php endif; if(C("sys_tenderMultiplicand")> 0): ?>,并且是<?php echo (C("sys_tenderMultiplicand")); ?>的倍数<?php endif; ?>)</span>
...
<script type="text/javascript">
	...
	jQuery(function(){
		$("#J_bid_submit").click(function(){
			if($.trim($("#J_BIDMONEY").val())=="" || !$.checkNumber($("#J_BIDMONEY").val())<?php if(C("sys_tenderMultiplicand")> 0): ?>|| parseFloat($("#J_BIDMONEY").val())%<?php echo (C("sys_tenderMultiplicand")); ?>!=0<?php endif; ?> || parseFloat($("#J_BIDMONEY").val())<=0 || parseFloat($("#J_BIDMONEY").val())<<?php echo ($loan["min_loan_money"]); ?> <?php if($loan["max_loan_money"] > 0): ?>|| parseFloat($("#J_BIDMONEY").val())><?php echo ($loan["max_loan_money"]); endif; ?>){
			...
			$.ajax({
				url:"<?php echo UU('Loan/index/NewbieBid');?>",
				data:query,
				type:"POST",
				dataType:"json",
